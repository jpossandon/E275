...
fnameEDF        = sprintf('s%02d_E275.EDF',str2num(s_n)); % name can be only 8 letters long
...
load([exp_path 'E275_randomization.mat'])
...
for nT = 1:nTrials
    ...
    if s_rand(nT).block_start == 1   
        if nT ==1 && s_rand(nT).block == 0          % uncrossed
          draw_instructions_and_wait('Beginn des Experiments \n Test Block \n Die H??nde bitte parallel positionieren (parallel). \n Zum Beginnen die Leertaste dr??cken (press Space)',win.bkgcolor,win.hndl,1)
        elseif nT ==1 && s_rand(nT).block == 1      % crossed, this according to randomization should be unnecesary
          draw_instructions_and_wait('Beginn des Experiments\n Test Block \n  F??r den n??chsten Block bitte die H??nde ??berkreuzen (crossed). \n Zum Fortfahren die Leertaste dr??cken. (press Space)',win.bkgcolor,win.hndl,1)
        elseif s_rand(nT).block == 0                % uncrossed
          draw_instructions_and_wait(sprintf('Block %d/17 beendet \n Pause \n F??r den n??chsten Block die H??nde bitte parallel positionieren (parallel). \n Zum Fortfahren die Leertaste dr??cken (press Space). \n',b),win.bkgcolor,win.hndl,1)
        elseif s_rand(nT).block == 1                % crossed
          draw_instructions_and_wait(sprintf('Block %d/17 beendet \n Pause  \n F??r den n??chsten Block die H??nde bitte ??berkreuzen (crossed). \n Zum Fortfahren die Leertaste dr??cken. (press Space) \n',b),win.bkgcolor,win.hndl,1)
        end      
        ...
        if s_rand(nT).block==0
            ...
            DrawFormattedText(win.hndl, 'F??r den n??chsten Block die H??nde parallel positionieren. Zum Fortfahren die Leertaste dr??cken (press Space).','center','center',255,55);
        elseif s_rand(nT).block==1
            DrawFormattedText(win.hndl, 'F??r den n??chsten Block die H??nde ??berkreuzen. Zum Fortfahren die Leertaste dr??cken (press Space).','center','center',255,55);
        end
        Screen('Flip', win.hndl);
        waitForKB_linux({'space'});
    end
