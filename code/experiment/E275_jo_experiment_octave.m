...
if win.isoctave
  ok = E275_jo_define_tact_states_octave(obj, win.on_time, win.cycle_time);         % E275_define_tact_states defines the codes to control the stimulation box via the parallel port:
else
...
if rem(eval(win.s_n),2)                                                           % we balance across subjects (according to their subject number) whether they start the experiment with the hand crossed or uncrossed, again this should not matter that much
