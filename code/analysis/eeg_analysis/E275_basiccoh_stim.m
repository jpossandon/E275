...
clear
for tk = 1:2; % subject number

    % Analysis parameters
    p.times_tflock              = [900 900];
    ...
    p.cfgTFR.toi                = (-p.times_tflock(1):25:p.times_tflock(2))/1000;	
    ...
    [TFR_LU] = getERPsfromtrl({cfg_eeg},{trls.trlLU},p.bsl,p.reref,p.analysis_type{at},p.keep);
...
%     [freq]          = ft_freqanalysis(p.cfgTFR, TFR_LU.ICAem);
            freq = TFR_RU.ICAem
    cfgcoh              = [];
    cfgcoh.method       = 'coh';
    coh                 = ft_connectivityanalysis(cfgcoh,TFR_RU.ICAem);
            
    cfgcoh              = [];
    cfgcoh.method       = 'coh';
    cfgcoh.complex      = 'imag';
    allcohim = ft_connectivityanalysis(cfgcoh,freq);
